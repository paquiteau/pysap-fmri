...
class SequentialFMRIReconstructor(BaseFMRIReconstructor):
    ...
    def reconstruct(self, kspace_data, x_init=None, max_iter_per_frame=15, reset_opt=True, recompute_smaps=False, smaps_kwargs=None,warm_x=True):
        ...
        for i in range(len(kspace_data)):
            ...
            if reset_opt:
                opt = self.initialize_opt(grad_op, x_init=next_init,synthesis_init=False,  opt_kwargs={"cost":None}, metric_kwargs=dict())
...
class ParallelFMRIReconstructor(SequentialFMRIReconstructor):
    ...
    def reconstruct(self, kspace_data, x_init=None, max_iter_per_frame=15,n_jobs=3, smaps_kwargs=None):
        ...
        def oneframe(kspace_data, idx, x_init, fourier_kwargs, grad_kwargs, smaps_kwargs, linear_op, prox_op, opt_name):
            ...
            if smaps_kwargs:
                Smaps, _ = get_Smaps(kspace_data,
                                            ...
                                            min_samples=fourier_op.samples.min(axis=0),
                                            max_samples=fourier_op.samples.max(axis=0),
            ...
            grad_op=grad_cls(fourier_op=fourier_op, **grad_kwargs,num_check_lips=20)

            opt = initialize_opt(opt_name, grad_op, linear_op, prox_op, x_init=x_init,
                                 synthesis_init=False, opt_kwargs={"cost":None}, metric_kwargs=dict())
            opt.iterate(max_iter=max_iter_per_frame)
        ...
        if (getattr(self.fourier_op.impl,'uses_sense', False) or self.smaps is not None) and x_init is None:
            x_init = np.squeeze(np.zeros(self.fourier_op.shape,dtype="complex64"))
        elif x_init is None:
            x_init = np.squeeze(np.zeros((self.fourier_op.n_coils, *self.fourier_op.shape),dtype="complex64"))
        ...
        fourier_kwargs={attr: copy.deepcopy(getattr(self.fourier_op,attr)) for attr in fourier_attrs}
        ...
        results = Parallel(n_jobs=n_jobs,verbose=self.verbose)(delayed(oneframe)(kspace_data, j,
                                                     x_init,
                                                     fourier_kwargs, grad_kwargs, smaps_kwargs,

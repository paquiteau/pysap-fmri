...
def add_temporal_gaussian_noise(array, sigma=1, g_factor_map=None, rng=None):
    ...
    g_noise = sigma * rng.standard_normal(shape, dtype=dtype)
    if g_factor_map is None:
        g_factor_map = np.ones(shape[:-1])

    if np.iscomplex(array).any():
        g_noise += 1j * sigma * rng.standard_normal(shape, dtype=dtype)
    return array + (g_noise * g_factor_map).astype(dtype)
